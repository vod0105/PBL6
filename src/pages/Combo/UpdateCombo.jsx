...
const UpdateCombo = ({ url }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${url}/api/v1/public/combo/${id}`);
        console.log("res", response.data.data[0].comboName);
        setData({
  ...
  //
  useEffect(() => {
    console.log("data11111", data.comboName);
    console.log("id", id);
  });
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      const response = await axios.put(
        `${url}/api/v1/admin/combo/update/${id}`,
        formData,
        { headers }
      );
      if (response.data.message) {
        ...
        setImage(null);
        toast.success("Update Combo Success");
        setTimeout(() => {
          navigate("/admin/Combo"); // Chuyển hướng tới trang mới
        }, 1000);
      } else {
