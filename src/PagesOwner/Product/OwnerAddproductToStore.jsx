...
import "./OwnerAddProductToStorev2.css";
...
const OwnerAddProducToStore = ({ url }) => {
  ...
  useEffect(() => {
    if (token) {
      ...
      axios
        .get(`${url}/api/v1/owner/stores`, { headers })
        .then((response) => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    console.log("aaaa", SelectedStoreId, selectedIds, 1);
    formData.append("storeId", SelectedStoreId);
    formData.append("productIds", selectedIds);
    formData.append("quantity", 1);
    try {
      const response = await axios.post(
        `${url}/api/v1/owner/products/apply-list-products-to-store`,
        formData,
        { headers }
      );
      ...
      <div className="right">
        ...
        <div className="action">
          ...
          {/* <div className="action-left action-btn">
            <button onClick={onSubmitHandler}>Thêm vào tất cả cửa hàng </button>
          </div> */}
...
export default OwnerAddProducToStore;
