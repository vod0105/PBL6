...
class StoreListState extends State<StoreList> {
  ...
  Future<List<LatLng>?> getRoute(
      LatLng startPoint, LatLng endPoint, bool show) async {
    Get.snackbar("Thông báo", "Vui lòng chờ để lấy dữ liệu",
        duration: const Duration(microseconds: 500));
    const apiKey = '5b3ce3597851110001cf62482f6aa59251a040bca10bfec215ef276c';
    final url =
        'https://api.openrouteservice.org/v2/directions/cycling-road?api_key=$apiKey&start=${startPoint.longitude},${startPoint.latitude}&end=${endPoint.longitude},${endPoint.latitude}';
    ...
    if (response.statusCode == 200) {
      ...
      if (!show) {
        Get.snackbar("Thông báo", "Lấy dữ liệu hoàn tất",
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return Dialog(
            ...
            child: StatefulBuilder(builder: (builder, setStateBuilder) {
              return Container(
  ...
  void _showRoute(LatLng start, LatLng end, bool show) async {
    routePoints = await getRoute(start, end, show) as List<LatLng>;
  }
                          ...
                          Row(
                            children: [
                              ...
                              SizedBox(
                                width: AppDimention.size100 * 2.5,
                                child: Text(
                                  "${formatTime(storecontroller.storeList[index].openingTime!)} - ${formatTime(storecontroller.storeList[index].closingTime!)}",
                                ),
                              )
                            ],
                          ),
